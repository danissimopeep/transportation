...
<@common.page title="Управление аккаунтами">
        ...
        <div class="row g-2 align-items-center justify-content-between">
            <div class="col-5">
                <div class="row g-2 align-items-center justify-content-start">
                    <div class="col-auto">
                        <a class="btn btn-secondary" href="/admin/clients">Клиенты</a>
                    </div>
                    <div class="col-auto">
                        <a class="btn btn-primary disabled" href="/admin/managers">Менеджеры</a>
                    </div>
                </div>
            </div>
            <div class="col-7">
    ...
    <#if managers??>
        ...
        <div class="container mt-3">
            <table class="table table-striped table-hover table-sm">
                ...
                <#list managers as m>
                    <tr>
                        <th class="col-1" scope="row">${m_index + 1}.</th>
                        <td class="col-2">${m.account.username}</td>
                        <td class="col-2">${m.credentials}</td>
                        <td class="col-2">${m.telephone}</td>
                        <td class="col-1">
                            <form method="get" action="/admin/message/${m.account.id}">
                                <input type="submit" class="btn btn-sm btn-primary" value="Сообщение">
                                <input type="hidden" name="_csrf" value="${_csrf.token}" />
                            </form>
                        </td>
                        <td class="col-1">
                            <form method="post" action="/admin/accounts/${m.account.id}">
                                <#if m.account.isActive()>
        ...
        <form method="post" id="newsletterForm" action="/admin/managers/newsletter">
    ...
    <script>
        ...
        function onSearch() {
            ...
            var request = search == null? "//localhost:8080/admin/managers" :
                "//localhost:8080/admin/managers?search="+search;
